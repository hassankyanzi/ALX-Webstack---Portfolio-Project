...
function checkemail() {
    try {
        ...
        if ($records > 0) {
            header("location:../register.php?p=$role&r=0927"); // Email already exists
        } else {
            if ($account_type == "101") {
...
function register_as_employee() {
    try {
        ...
        $email = $_POST['email'];
        $login = md5($_POST['password']); // Hash the password
        ...
        $stmt->execute();
        header("location:../register.php?p=Employee&r=1123&message=success"); // Redirect with success message
    } catch (PDOException $e) {
...
function register_as_employer() {
    try {
        ...
        $stmt->execute();
        header("location:../register.php?p=Employer&r=1123&message=success"); // Redirect with success message
    } catch (PDOException $e) {
